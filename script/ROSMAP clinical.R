...
clinical_dat <- clinical_dat %>%
  mutate(apoe4 = factor(apoe_genotype %in% c("24", "34", "44"), labels = c("APOE4-", "APOE4+")),
         ...
         braaksc_grouped = factor(
           case_when(braaksc %in% c("None", "Stage I", "Stage II") ~ "None-Stage II",
                     braaksc %in% c("Stage III", "Stage IV") ~ "Stage III-IV",
                     braaksc %in% c("Stage V", "Stage VI") ~ "Stage V-VI",
                     TRUE ~ NA_character_), 
           levels = c("None-Stage II", "Stage III-IV", "Stage V-VI")),
  ...
  filter(!is.na(apoe4), !is.na(braaksc_grouped), !is.na(ceradsc), !is.na(cogdx), !is.na(msex), !is.na(age_death_num)) %>%
  dplyr::select(projid, apoe4, apoe_genotype, braaksc, braaksc_grouped, ceradsc, cogdx, msex, age_death_num)
...
demographic <- clinical_dat %>%
  dplyr::select(msex, cogdx, age_death_num, apoe_genotype) %>%
  tbl_summary(
    by = cogdx,
    statistic = list(
...
for (dx in c("NCI", "MCI", "AD")) {
  ...
  fit <- polr(braaksc_grouped ~ apoe4 + age_death_num + msex, # adjust for age and sex
...
for (dx in c("NCI", "MCI", "AD")) {
  ...
  fit <- polr(ceradsc ~ apoe4 + age_death_num + msex, # adjust for age and sex
              data = sub, Hess = TRUE, method = "logistic")
