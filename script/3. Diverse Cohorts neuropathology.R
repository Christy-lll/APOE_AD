...
clinical_dat <- clinical_dat %>%
  ...
  mutate(apoe4 = factor(apoeGenotype %in% c("24", "34", "44"), labels = c("APOE ε4-", "APOE ε4+")),
         ...
         ADoutcome = factor(ADoutcome, levels = c("Control", "AD"), labels = c("NCI", "AD")),
...
braak <- ggplot(braak_plot_dat, aes(x = apoe4, y = prop, fill = Braak)) +
  ...
  labs(title = "Braak stage distribution in AMP-AD Diverse Cohorts Study",
...
cerad <- ggplot(cerad_plot_dat, aes(x = apoe4, y = prop, fill = amyCerad)) +
  ...
  labs(title = "CERAD score distribution in AMP-AD Diverse Cohorts Study",
...
for (dx in c("NCI", "AD")) {
  ...
  fit <- polr(Braak ~ apoe4 + ageDeath_num + sex,
